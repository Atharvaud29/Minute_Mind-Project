...
//   const [form, setForm] = useState({ issue: '', raised_by: '', resolution: '', severity: 'Low' })
//   return (
...
export default function ConflictDetection() {
  ...
  const updateConflictMutation = useMutation({
    mutationFn: ({ id, payload }) => updateConflict(id, payload),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['conflicts'] })
  })
  ...
  const handleAddConflict = () => {
    if (!form.issue) return alert('Please enter the conflict issue.')
    addConflict.mutate(form)
    setForm({ issue: '', raised_by: '', resolution: '', severity: 'Low' })
  }
      ...
      <div className="card p-4 space-y-3">
        <div className="grid md:grid-cols-2 gap-3">
          <input
            ...
            placeholder="Issue"
            className="border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-rose-400"
          />
          <input
        ...
        <input
          ...
          className="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-rose-400"
        />
      </div>
            ...
            <div className="flex items-start justify-between">
              ...
              <span
                className={`badge ${
                  ...
                  c.severity === 'Medium' ? 'bg-accent-100 text-accent-700' :
